...
test("@claim:paid-tier-copy keeps a $29 Plus purchase from opening a known unavailable checkout", async ({ page }) => {
  const checkoutUrl = "https://api.sociobot.in/api/v1/products/reminder-mailroom/checkout";
  const checkoutRequests: string[] = [];
  page.on("request", (request) => { if (request.url() === checkoutUrl) checkoutRequests.push(request.url()); });
  await page.goto("/");
  ...
  await paid.getByText("Checkout is being enabled").click({ force: true });
